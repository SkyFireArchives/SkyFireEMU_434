...
void Spell::SelectImplicitTrajTargets()
{
    ...
    std::list<WorldObject*> targets;
    Skyfire::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetSrc(), m_caster, m_spellInfo);
    ...
    SearchTargets<Skyfire::WorldObjectListSearcher<Skyfire::WorldObjectSpellTrajTargetCheck> > (searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetSrc(), dist2d);
    if (targets.empty())
        return;
