...
Location MoveSpline::ComputePosition() const
{
    ...
    if (splineflags.animation)
        ;// MoveSplineFlag::Animation disables falling or FixedOrientation movement
    else if (splineflags.fixedorientation)
        computeFixedOrientationElevation(c.z);
    else if (splineflags.falling)
        computeFallElevation(c.z);

    if (splineflags.done && splineflags.isFacing())
    {
        if (splineflags.finalangle)
            c.orientation = facing.angle;
        else if (splineflags.finalpoint)
            c.orientation = atan2(facing.f.y-c.y, facing.f.x-c.x);
        //nothing to do for MoveSplineFlag::FinalTarget flag
    }
    else
    {
...
void MoveSpline::computeFixedOrientationElevation(float& el) const
{
...
void MoveSpline::Initialize(const MoveSplineInitArgs& args)
{
    ...
    // init FixedOrientation / animation
    // spline initialized, duration known and i able to compute FixedOrientation acceleration
    if (args.flags & (MoveSplineFlag::FixedOrientation | MoveSplineFlag::Animation))
    {
        effect_start_time = Duration() * args.time_perc;
        if (args.flags.FixedOrientation && effect_start_time < Duration())
        {
            float f_duration = MSToSec(Duration() - effect_start_time);
            vertical_acceleration = args.fixedorientation_amplitude * 8.f / (f_duration * f_duration);
        }
    }
}
