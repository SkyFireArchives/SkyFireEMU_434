...
void WorldSession::HandleGuildRosterOpcode(WorldPacket& recvPacket)
{
    ...
    recvPacket.ReadXorByte(mask[2], bytes[3]);
    recvPacket.ReadXorByte(mask[6], bytes[0]);
    recvPacket.ReadXorByte(mask[5], bytes[5]);
    recvPacket.ReadXorByte(mask[0], bytes[2]);
    recvPacket.ReadXorByte(mask[3], bytes[7]);
