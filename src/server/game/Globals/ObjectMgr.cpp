...
void ObjectMgr::LoadQuests()
{
    ...
    for (QuestMap::iterator iter = _questTemplates.begin(); iter != _questTemplates.end(); ++iter)
    {
        ...
        if (qinfo->Flags & ~QUEST_SKYFIRE_FLAGS_DB_ALLOWED)
        {
            sLog->outErrorDb("Quest %u has `SpecialFlags` = %u > max allowed value. Correct `SpecialFlags` to value <= %u",
                qinfo->GetQuestId(), qinfo->Flags  >> 20, QUEST_SKYFIRE_FLAGS_DB_ALLOWED >> 20);
            qinfo->Flags &= QUEST_SKYFIRE_FLAGS_DB_ALLOWED;
        }
            ...
            if (id)
            {
                ...
                if (!qinfo->RequiredNpcOrGo[j])
                {
                    ...
                    if (found)
                    {
                        ...
                        {
                            sLog->outErrorDb("Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT for quest %u and RequiredNpcOrGo%d = 0, but quest not have flag QUEST_SKYFIRE_FLAGS_EXPLORATION_OR_EVENT. Quest flags or RequiredNpcOrGo%d must be fixed, quest modified to enable objective.", spellInfo->Id, qinfo->Id, j+1, j+1);
...
void ObjectMgr::LoadGameObjectTemplate()
{
    ...
    do
    {
        ...
        got.unkInt32 = 0;//fields[48].GetInt32();
